...
// /*Function prototypes */
// __device__ struct par_rbNode * newNode(int);
...
// __deivce__ struct par_rbNode *nodes[M]; need M pointers
...
// __device__ struct par_rbNode *newNode(int key){
...
// 	atomicCAS(&createFlag,false,true); //Release the lock
...
__device__ struct par_rbNode * Traverse(int key){
	...
	do{
		savert = root;
		atomicCAS(&root->flag,false,true); //Catching the flag of the root
	}while(savert!=root);
...
__global__ void RBT(struct par_rbNode *d_nodes) {
	...
	// unsigned threadCount = gridDim.x*blockDim.x;
...
int main() {
	...
	for(int i=0;i<M;i++){
		struct par_rbNode *newNode = (struct par_rbNode *)malloc(sizeof(struct par_rbNode));
		h_nodes[i] = newNode;
