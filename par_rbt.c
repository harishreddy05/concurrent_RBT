					...
					if(ok){
						...
						if(ok){
							...
							if(ok){
								...
								if(ok){
									ok = atomicCAS(&savegp->right,uncle,uncle);
								}
								if(!ok){
									savep->flag = false;
									savegp->flag = false;
...
__device__ void clocked_printInorder(struct par_rbNode* node)
{
...
int main(){
	...
	//allocate memory for in GPU , change all the malloc statements
